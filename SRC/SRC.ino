...
void configWebPages(void) {
  ...
  server.on("/ssidList.json", HTTP_GET, [](AsyncWebServerRequest * request) {
    request->send(SD, "/ssidList.json");
  });
...
void getWifi(void) {
  if (cmg.getOPmode() == 1) {
    ...
    if (n == 0) {
      ...
      for (int i = 0; i < n; ++i) {
        StaticJsonDocument<512> redesWifi;
        redesWifi["ssid"] = WiFi.SSID(i);
        redesWifi["val"] = WiFi.SSID(i);
        char buffer[512];
        serializeJsonPretty(redesWifi, buffer);
        if(i == 0){
          SSIDs = ("[");
        }
        if(i <= n-2){
